...
func ethtoolPort(port string) *ethPort {
	...
	// 获取端口队列长度
	byteRingPre, err := run.Command("sh", "-c", fmt.Sprintf("ethtool -g %s|sed -n '/Pre-set/,/Current/p'|egrep -v 'Pre-set|Current'", port))
	if err == nil {
		lines := strings.Split(string(byteRingPre), "\n")
		parseRingBuffer(ret, lines, true)
	}
	byteRingCur, err := run.Command("sh", "-c", fmt.Sprintf("ethtool -g %s|sed -n '/Current/,//p'|egrep -v 'Current'", port))
	if err == nil {
	...
	// 获取端口队列数
	byteChannelPre, err := run.Command("sh", "-c", fmt.Sprintf("ethtool -l %s|sed -n '/Pre-set/,/Current/p'|egrep -v 'Pre-set|Current'", port))
	if err == nil {
		lines := strings.Split(string(byteChannelPre), "\n")
		parseChannel(ret, lines, true)
	}
	byteChannelCur, err := run.Command("sh", "-c", fmt.Sprintf("ethtool -l %s|sed -n '/Current/,//p'|egrep -v 'Pre-set|Current'", port))
	if err == nil {
	...
	// 获取端口pcie 和 驱动信息
	byteDrive, err := run.Command("sh", "-c", fmt.Sprintf("ethtool -i %s", port))
	if err == nil {
	...
	// 获取端口设置
	byteSetting, err := run.Command("sh", "-c", fmt.Sprintf("ethtool %s", port))
	if err == nil {
...
func ethDriver(ret *ethPort, msgSlice []string) {
	...
	if ret.PCIAddr == "0000:00:00.0" {
		...
		ret.PCI = *utils.GetPCIe(ret.PCIAddr)
	}
}
