...
func parsePort(port string) netPort {
	...
	for _, sub := range subDir {
		if sub.Name() == "device" {
			...
			if utils.IsEmptyValue(reflect.ValueOf(ret.LLDP)) && ret.Driver.DriverName == "i40e" {
				checkI40e(port, &ret)
				ret.LLDP = lldpctl(port)
			}
		} else if sub.Name() == "address" {
			addr, err := utils.ReadFile(filepath.Join(dir, "address"))
			if err != nil {
	...
	if !utils.IsEmptyValue(reflect.ValueOf(ipv4Info)) {
		ret.IPv4 = ipv4Info
	}
	return ret
}
...
func checkI40e(port string, ret *netPort) {
	byteFW, err := run.Command("sh", "-c", fmt.Sprintf("ethtool --show-priv-flags %s|awk -F: '/disable-fw-lldp/{print $2}'", port))
	if err != nil {
		log.Printf("failed to get %s fw-lldp info: %v", port, err)
		return
	}
	if strings.TrimSpace(string(byteFW)) == "off" {
		kernel, err := run.Command("uname", "-r")
		if err != nil {
			log.Printf("failed to get kernel version: %v", err)
			return
		}
		if strings.HasPrefix(string(kernel), "3.16") {
			run.Command(fmt.Sprintf("echo 'lldp stop' > /sys/kernel/debug/i40e/%s/command", ret.PCIAddr))
		}
		run.Command("sh", "-c", fmt.Sprintf("ethtool --set-priv-flags %s disable-fw-lldp on 2>/dev/null", port))
		time.Sleep(time.Second * 30)
	}
}

func lldpctl(port string) ethLLDP {
	byteLLDP, err := run.Command("sh", "-c", fmt.Sprintf("lldpctl %s -f keyvalue", port))
	if err != nil {
		log.Printf("failed to get %s lldp info.", port)
		return ethLLDP{}
	}
	ret := ethLLDP{}
	lines := utils.SplitAndTrim(string(byteLLDP), "\n")
...
func getIPv4(port string) ipv4 {
	ret := ipv4{}
	byteIP4, err := run.Command("sh", "-c", fmt.Sprintf("ip addr show %s | awk '/inet /{print $2}'", port))
	if err != nil {
		println("failed to get ipv4 info: ", err)
	}
	lines := utils.SplitAndTrim(string(byteIP4), "\n")
	if len(lines) == 0 {
		return ret
	}
	for _, line := range lines {
...
func getBond(ret *NETWORK) {
	...
	for _, file := range fileList {
		res := bond{}
		res.Name = file.Name()
		info, err := utils.ReadLines(fmt.Sprintf("%s/%s", bondDir, res.Name))
		if err != nil {
		...
		for _, line := range info {
			...
			if sFlag {
				...
				case "Slave Interface":
					if !utils.IsEmptyValue(reflect.ValueOf(sRes)) {
