...
func Mdadm(pciAddr string, num int) RHController {
	_, err := run.Command("sh", "-c", fmt.Sprintf("/usr/sbin/mdadm --detail-platform | grep %s", pciAddr))
	if err != nil {
		log.Printf("failed to get mdadm  %s info: %v \n", pciAddr, err)
		return RHController{}
	}
	byteCTRL, err := run.Command("sh", "-c", "/usr/sbin/mdadm --detail-platform | grep -v ^$")
...
func parseVROC(data []byte, ret *RHController, diskMap *map[string]physicalDrive) {
	lines := utils.SplitAndTrim(string(data), "\n")
...
func vrocLogicalDrive(cid string, diskMap map[string]physicalDrive) []logicalDrive {
	file := `/proc/mdstat`
	byteLD, err := run.Command("sh", "-c", fmt.Sprintf("awk '/: active/{print $1}' %s", file))
	if err != nil {
	...
	for i, ld := range sliceLD {
		...
		byteLDInfo, err := run.Command("sh", "-c", fmt.Sprintf("/usr/sbin/mdadm --detail %s | grep -v ^$", res.MappingFile))
		if err != nil {
...
func parsePhysicalDrive(device string) physicalDrive {
	ret := physicalDrive{}
	byteSMART, err := run.Command("sh", "-c", fmt.Sprintf("/usr/sbin/smartctl -d %s sat -a -j | grep -v '^$|#'", device))
	if err != nil {
...
func parseVROCNVMe(nvmeDir string) map[string]physicalDrive {
	...
	for _, name := range nameSlice {
		...
		byteSmart, err := run.Command("sh", "-c", fmt.Sprintf("/usr/sbin/smartctl %s -d nvme -a -j | grep -v ^$", deviceName))
		if err != nil {
