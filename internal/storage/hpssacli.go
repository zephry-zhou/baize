...
func Hpssacli(pci string, ctrNum int) RHController {
	...
	if !utils.PathExistsWithContent(hpasscli) {
		log.Println("hpssacli not exists")
		return RHController{}
	}
	ret := RHController{}
	for i := 0; i <= ctrNum; i++ {
		pciSlot, err := run.Command("sh", "-c", fmt.Sprintf("%s ctrl slot=%s show  | grep %s", hpasscli, strconv.Itoa(i), pci))
...
func parseCtr(ret *RHController, cmdHeader string) {
	ctrByte, err := run.Command("sh", "-c", fmt.Sprintf("%s show | grep -v ^$", cmdHeader))
	if err != nil {
		log.Println("failed to get RAID/HBA info in:", cmdHeader)
		return
	}
	lines := utils.SplitAndTrim(string(ctrByte), "\n")
...
func parseEnclosure(cmdHeader string) []backplate {
	byteEnc, err := run.Command("sh", "-c", fmt.Sprintf("%s enclosure all show | grep 'at Port'", cmdHeader))
	if err != nil {
	...
	lines := utils.SplitAndTrim(string(byteEnc), "\n")
	for _, line := range lines {
		...
		res.State = matches[3]
		byteEid, err := run.Command("sh", "-c", fmt.Sprintf("%s enclosure %s show | awk -F: '/Drive Bays/{print $2}'", cmdHeader, res.Eid))
		if err != nil {
...
func parseDrv(cmdHeader string) map[string][]physicalDrive {
	byteDrv, err := run.Command("sh", "-c", fmt.Sprintf("%s pd all show | awk '/physicaldrive/{print $2}'", cmdHeader))
	if err != nil {
		log.Println("failed to get RAID/HBA disk info:", cmdHeader)
		return map[string][]physicalDrive{}
	}
	drvList := utils.SplitAndTrim(string(byteDrv), "\n")
	...
	for _, drv := range drvList {
		...
		pd := physicalDrive{}
		pd.Location = drv
		byteDrvInfo, err := run.Command("sh", "-c", fmt.Sprintf("%s pd %s show | grep -v ^$", cmdHeader, drv))
		if err != nil {
			log.Println("failed to get RAID/HBA disk detail info:", cmdHeader)
			continue
		}
		lines := utils.SplitAndTrim(string(byteDrvInfo), "\n")
		key := lines[1]
...
func parseDrvInfo(pd *physicalDrive, lines []string) {
	...
	dev := "sda"
	byteDev, err := run.Command("sh", "-c", `lsblk -d -o NAME | grep -v NAME`)
	if err != nil {
	...
	for _, line := range lines {
		...
		case "Logical/Physical Block Size":
			block := utils.SplitAndTrim(value, "/")
	...
	if strings.ToUpper(pd.State) == "OK" {
		...
		byteSMART, err := run.Command("sh", "-c", cmd)
		if err != nil {
...
func parseLD(cmdHeader string, cid string, diskMap map[string][]physicalDrive) []logicalDrive {
	byteLD, err := run.Command("sh", "-c", fmt.Sprintf("%s ld all show | awk '/logicaldrive/{print $2}'", cmdHeader))
	if err != nil {
		log.Printf("failed to get logical drive info: %v \n", err)
		return []logicalDrive{}
	}
	sliceLD := utils.SplitAndTrim(string(byteLD), "\n")
	ret := []logicalDrive{}
	for _, ld := range sliceLD {
		...
		byteLDInfo, err := run.Command("sh", "-c", fmt.Sprintf("%s ld %s show detail| grep -v ^$", cmdHeader, ld))
		if err != nil {
			log.Printf("failed to get logical drive detail info: %v \n", err)
			continue
		}
		lines := utils.SplitAndTrim(string(byteLDInfo), "\n")
		res.DG = lines[1]
...
func parseLDInfo(ld *logicalDrive, lines []string) {
	for _, line := range lines {
		fileds := utils.SplitAndTrim(line, ":")
		if len(fileds) != 2 {
			continue
		}
		key := fileds[0]
