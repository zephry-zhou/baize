...
func parseSMART(ret *physicalDrive, s []byte) {
	...
	for key, value := range v {
		...
		case "nvme_total_capacity":
			cap, unit, err := utils.Unit2Human(value.(float64), "B", true)
			if err != nil {
				log.Fatalln("converion disk size failed: ", err)
			}
			ret.Capacity = strings.Join([]string{fmt.Sprintf("%.2f", cap), unit}, " ")
		case "user_capacity":
			cap, unit, err := utils.Unit2Human(value.(map[string]interface{})["bytes"].(float64), "B", true)
			if err != nil {
				log.Fatalln("converion disk size failed: ", err)
			}
			ret.Capacity = strings.Join([]string{fmt.Sprintf("%.2f", cap), unit}, " ")
...
func diskManufacturer(s string) (string, string) {
	...
	devmap := utils.ReadJSONFile(filePath)
	sl := strings.Split(s, " ")
