...
func GetNVMeDevices() []nvme {
	byteNvmes, err := run.Command("sh", "-c", `lspci -Dnn | egrep '\[0108\]' | awk '{print $1}'`)
	if err != nil {
	...
	lines := utils.SplitAndTrim(string(byteNvmes), "\n")
	ret := []nvme{}
	for _, line := range lines {
		res := nvme{}
		res.PCI = *utils.GetPCIe(line)
...
func parseNVMePCIDirectory(pciAddr string, ret *nvme) {
	...
	for _, name := range nameSlice {
		...
		byteSmart, err := run.Command("sh", "-c", fmt.Sprintf("/usr/sbin/smartctl %s -d nvme -a -j | grep -v ^$", ret.MappingFile))
		if err != nil {
